...
class control {
  ...
  createControlService(device, type) {
    ...
    if (this.platform.useFahrenheit) {
      currentAmps = (
        ((device.maxAvailableCurrent - 32 + 0.01) * 5) /
        9
      ).toFixed(2);
    } else {
  ...
  configureControlService(device, controlService) {
    ...
    this.log.debug(
      'configured %s control for %s',
      controlService.getCharacteristic(Characteristic.Name).value,
      device.name
    );
    controlService
  ...
  updateControlService(controlService, controlState, controlLimit) {
    ...
    controlService
      ...
      .updateValue(controlState);
    controlService
      .getCharacteristic(Characteristic.TargetHeatingCoolingState)
      .updateValue(controlState);
    controlService
      .getCharacteristic(Characteristic.CurrentTemperature)
      .updateValue(controlLimit);
    controlService
      .getCharacteristic(Characteristic.TargetTemperature)
      .updateValue(controlLimit);
  }
  async setControlAmps(device, controlService, value, callback) {
    ...
    let currentMode;
    if (
      controlService.getCharacteristic(Characteristic.StatusFault).value ==
      ...
      let chargerData = await this.wallboxapi
        .getChargerData(this.platform.token, device.id)
        .catch((err) => {
          this.log.error('Failed to get charger data. \n%s', err);
        });
      try {
        statusCode = chargerData.status;
        currentMode = this.enumeration.items.filter(
      ...
      switch (currentMode) {
          ...
          switch (statusCode) {
            case 209:
              ...
              controlService
                .getCharacteristic(Characteristic.TargetTemperature)
                .updateValue(
                  controlService.getCharacteristic(
        ...
        case 'chargingMode':
          ...
          if (
            controlService.getCharacteristic(Characteristic.StatusFault)
              .value == Characteristic.StatusFault.GENERAL_FAULT
          ) {
            callback('error');
          } else {
            let response = await this.wallboxapi
              .setAmps(this.platform.token, device.id, amps)
              .catch((err) => {
            ...
            switch (response.status) {
              ...
              default:
                ...
                this.log.info(
                  'Failed to change charging amps %s',
                  response.data.title
                );
        ...
        case 'errorMode':
          this.log.info(
            'This opertation cannot be completed at this time, status %s',
            statusCode
          );
          this.log.error(
            'the charger %s has a fault condition with code=%s',
            device.name,
            statusCode
          );
          controlService
  ...
  async setControlState(device, controlService, value, callback) {
    ...
    if (
      ...
      try {
        ...
        this.log.debug(
          'checking status code = %s, current mode = %s',
          statusCode,
          currentMode
        );
      } catch (error) {
      ...
      switch (currentMode) {
        ...
        case 'standbyMode':
          ...
          if (
            ...
            let response = await this.wallboxapi
              .remoteAction(this.platform.token, device.id, 'resume')
              .catch((err) => {
        ...
        case 'chargingMode':
          ...
          if (
            ...
            let response = await this.wallboxapi
              .remoteAction(this.platform.token, device.id, 'pause')
              .catch((err) => {
  ...
  getControlState(controlService, callback) {
    if (
      ...
      let currentValue = controlService.getCharacteristic(
        Characteristic.CurrentHeatingCoolingState
      ).value;

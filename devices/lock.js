...
class lockMechanism {
  ...
  createLockAccessory(device, config, uuid, platformAccessory) {
    ...
    platformAccessory
      ...
      .setCharacteristic(
        ...
        config.software.currentVersion
      )
  ...
  createLockService(device) {
    this.log.debug(
      'create lock service for %s, serial number %s',
      device.name,
      device.serialNumber
    );
    let lockService = new Service.LockMechanism(device.name, device.id);
  ...
  configureLockService(device, lockService) {
    this.log.debug(
      'configured %s lock for %s',
      lockService.getCharacteristic(Characteristic.Name).value,
      device.name
    );
    lockService
  ...
  updateLockService(lockService, lockStatusFault, lockInUse, lockedState) {
    if (!lockService) {
      return this.log.error('No lock configured');
    }
    this.log.debug(
      'Update Lock for %s',
      lockService.getCharacteristic(Characteristic.Name).value
    ...
    lockService
      ...
      .updateValue(lockStatusFault);
    lockService
      .getCharacteristic(Characteristic.OutletInUse)
      .updateValue(lockInUse);
    lockService
      .getCharacteristic(Characteristic.LockCurrentState)
      .updateValue(lockedState);
    lockService
      .getCharacteristic(Characteristic.LockTargetState)
      .updateValue(lockedState);
  }
  async getLockCurrentState(device, lockService, callback) {
    let currentValue = lockService.getCharacteristic(
  ...
  setLockCurrentState(device, lockService, value, callback) {
    ...
    if (
      lockService.getCharacteristic(Characteristic.StatusFault).value ==
  ...
  async setLockTargetState(device, lockService, value, callback) {
    ...
    //await this.platform.getNewToken(this.platform.token);
      ...
      if (value == true) {
        ...
        let chargerId = lockService.getCharacteristic(
          Characteristic.Identifier
        ).value;
        let response = await this.wallboxapi
          .lock(this.platform.token, chargerId, value)
          .catch((err) => {

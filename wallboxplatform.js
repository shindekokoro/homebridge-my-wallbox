...
class wallboxPlatform {
  constructor(log, config, api) {
    ...
    this.showAPIMessages = config.showAPIMessages
      ? config.showAPIMessages
      : false;
    this.showUserMessages = config.showUserMessages
      ? config.showUserMessages
      : false;
    this.id;
    this.userId;
    this.model_name;
    ...
    if (!config.email || !config.password) {
      this.log.error(
        'Valid email and password are required in order to communicate with wallbox, please check the plugin config'
      );
    }
    this.log.info(
      'Starting Wallbox platform using homebridge API',
      api.version
    );
    if (api) {
      this.api = api;
  ...
  async getDevices() {
    try {
      ...
      // get signin & token
      let signin = await this.wallboxapi
      ...
      this.log.debug(
        'Found token  %s********************%s',
        signin.data.attributes.token.substring(0, 35),
        signin.data.attributes.token.substring(
          signin.data.attributes.token.length - 35
        )
      );
      this.log.debug(
        'Found refresh token  %s********************%s',
        signin.data.attributes.refresh_token.substring(0, 35),
        signin.data.attributes.refresh_token.substring(
          signin.data.attributes.refresh_token.length - 35
        )
      );
      ...
      this.ttlTime = Math.round(
        (signin.data.attributes.ttl - Date.now()) / 60 / 1000
      );
      if (this.showUserMessages) {
        this.log.info('Current time ', new Date(Date.now()).toLocaleString());
        this.log.info(
          'Token will expire on %s, %s minutes ',
          new Date(signin.data.attributes.ttl).toLocaleString(),
          Math.round((signin.data.attributes.ttl - Date.now()) / 60 / 1000)
        );
        this.log.info(
          'Refresh Token will expire on %s, %s days ',
          new Date(this.refreshTtl).toLocaleString(),
          Math.round((this.refreshTtl - Date.now()) / 24 / 60 / 60 / 1000)
        );
      } else {
      ...
      //get get user id
      let userId = await this.wallboxapi
        .getId(this.token, this.id)
        .catch((err) => {
          this.log.error('Failed to get userId for build. \n%s', err);
        });
      this.log.debug('Found user ID %s', userId.data.attributes.value);
      this.userId = userId.data.attributes.value;
      //get groups
      let groups = await this.wallboxapi
        .getChargerGroups(this.token)
        .catch((err) => {
      ...
      //get user
      let user = await this.wallboxapi
        .getUser(this.token, this.userId)
        .catch((err) => {
          this.log.error('Failed to get user for build. \n%s', err);
        });
      this.log.info(
      ...
      user.data.accessConfigs
        .filter((accessConfig) => {
          groups.result.groups.forEach((group) => {
            if (
              !this.locationName ||
              (this.locationName == group.name &&
                accessConfig.group == group.id)
            ) {
              this.log.info('Found device at the location: %s', group.name);
              this.locationMatch = true;
            } else {
              this.log.info(
                'Skipping device at %s, not found at the configured location: %s',
                group.name,
                this.locationName
              );
        ...
        .forEach((accessConfig) => {
          accessConfig.chargers.forEach(async (charger) => {
            //loop each charger
            let chargerData = await this.wallboxapi
              .getChargerData(this.token, charger)
              .catch((err) => {
                this.log.error(
                  'Failed to get charger data for build. \n%s',
                  err
                );
              });
            let chargerName = this.cars.filter(
              (car) => car.chargerName == chargerData.name
            );

            if (chargerName[0]) {
              let uuid = UUIDGen.generate(chargerData.uid);
              let chargerConfig = await this.wallboxapi
                .getChargerConfig(this.token, charger)
                .catch((err) => {
                  this.log.error(
                    'Failed to get charger configs for build. \n%s',
                    err
                  );
                });
              ...
              let batteryService =
                this.battery.createBatteryService(chargerData);
              let outletService = this.outlet.createOutletService(
                chargerData,
                'Start/Pause'
              );
              let controlService = this.control.createControlService(
                chargerData,
                'Charging Amps'
              );
              let switchService = this.basicSwitch.createSwitchService(
                chargerData,
                'Start/Pause'
              );
              ...
              if (this.showSensor) {
                ...
                lockAccessory
                  ...
                  .addLinkedService(sensorService);
              ...
              if (this.showBattery) {
                ...
                lockAccessory
                  .getService(Service.LockMechanism)
                  .addLinkedService(batteryService);
              ...
              if (this.showControls == 5 || this.showControls == 4) {
                ...
                lockAccessory
                  .getService(Service.LockMechanism)
                  .addLinkedService(outletService);
                lockAccessory.addService(outletService);
              }
              if (this.showControls == 3 || this.showControls == 4) {
                this.control.configureControlService(
                  chargerData,
                  controlService
                );
                lockAccessory
                  .getService(Service.LockMechanism)
                  .addLinkedService(controlService);
                lockAccessory.addService(controlService);
              }
              if (this.showControls == 1 || this.showControls == 4) {
                this.basicSwitch.configureSwitchService(
                  chargerData,
                  switchService
                );
                lockAccessory
                  .getService(Service.LockMechanism)
                  .addLinkedService(switchService);
              ...
              this.log.warn(
                '%s not found in config, not added.',
                chargerData.name
              );
            }
          });
      ...
      setTimeout(() => {
        this.log.info('Wallbox platform finished loading');
      }, 2500);
    } catch (err) {
      if (this.retryAttempt < this.retryMax) {
        this.retryAttempt++;
        this.log.error(
          'Failed to get devices. Retry attempt %s of %s in %s seconds...',
          this.retryAttempt,
          this.retryMax,
          this.retryWait
        );
    ...
    try {
      ...
      if (refresh.status == 200) {
        if (this.showUserMessages) {
          this.log.info(
            'Updated token  %s********************%s',
            this.token.substring(0, 35),
            this.token.substring(this.token.length - 35)
          );
          this.log.info(
            'Updated refresh token  %s********************%s',
            this.refreshToken.substring(0, 35),
            this.refreshToken.substring(this.refreshToken.length - 35)
          );
        } else {
          this.log.debug(
            'Updated token  %s********************%s',
        ...
        this.ttlTime = Math.round(
          (refresh.data.data.attributes.ttl - Date.now()) / 60 / 1000
        );
  ...
  setChargerRefresh(device) {
    ...
    setInterval(async () => {
      ...
      try {
        let checkUpdate = await this.wallboxapi
          .getChargerConfig(this.token, device.id)
          .catch((err) => {
        ...
        if (checkUpdate.software.updateAvailable) {
          this.log.warn(
            '%s software update %s is available',
            checkUpdate.name,
            checkUpdate.software.latestVersion
          );
        }
      } catch (err) {
  ...
  async startLiveUpdate(device) {
    ...
    if (delta > 500) {
      ...
      this.log.debug(
        'Skipped new live update due to duplicate call, timestamp delta %s ms',
        delta
      );
      return;
    }
  ...
  calcBattery(batteryService, energyAdded, chargingTime) {
    let wallboxChargerName = batteryService.getCharacteristic(
      Characteristic.Name
    ).value;
    try {
      if (this.cars) {
        let car = this.cars.filter(
          (charger) => charger.chargerName == wallboxChargerName
        );
        if (car[0]) {
          this.batterySize = car[0].kwH;
        } else {
          this.log.warn(
            'Unable to find charger named "%s" as configured in the plugin settings for car "%s" with charger "%s". Please check your plugin settings.',
            wallboxChargerName,
            this.cars[0].carName,
            this.cars[0].chargerName
          );
        }
      }
    } catch (err) {
    ...
    this.log.debug(
      'Charging time %s hours %s minutes, charge added %s kWh, %s%',
      hours,
      minutes,
      energyAdded,
      percentAdded
  ...
  async getStatus(id) {
    let statusResponse = await this.wallboxapi
      .getChargerStatus(this.token, id)
      .catch((err) => {
  ...
  async updateStatus(charger) {
    try {
      ...
      let controlService = lockAccessory.getServiceById(
        Service.Thermostat,
        chargerID
      );
      let switchService = lockAccessory.getServiceById(
        Service.Switch,
        chargerID
      );
      let outletService = lockAccessory.getServiceById(
        Service.Outlet,
        chargerID
      );
      let lockService = lockAccessory.getServiceById(
        Service.LockMechanism,
        chargerID
      );
      let batteryService = lockAccessory.getServiceById(
        Service.Battery,
        chargerID
      );
      let tempControl = this.useFahrenheit
        ? (((maxAmps - 32 + 0.01) * 5) / 9).toFixed(2)
        : maxAmps;
      let sensorService = lockAccessory.getServiceById(
        Service.HumiditySensor,
        chargerID
      );
      let chargerState;
      let statusInfo;
      let batteryPercent = this.calcBattery(
        batteryService,
        added_kWh,
        chargingTime
      );
      this.log.debug('Updating charger ID %s', chargerID);
      lockService = lockAccessory.getServiceById(
        Service.LockMechanism,
        chargerID
      );
      ...
      try {
        statusInfo = this.enumeration.items.filter(
          (result) => result.status == statusID
        )[0];
        this.log.debug(
          'Refreshed charger with status=%s %s - %s. %s.',
          statusID,
      ...
      switch (statusInfo.mode) {
        ...
        case 'readyMode':
          ...
          chargerState = false;
          this.lockMechanism.updateLockService(
            lockService,
          ...
          this.control.updateControlService(
            controlService,
            chargerState,
            tempControl
          );
          this.basicSwitch.updateSwitchService(switchService, chargerState);
          this.battery.updateBatteryService(
            batteryService,
        ...
        case 'errorMode':
          ...
          switch (statusID) {
            ...
            case 5: //Offline':
              this.log.warn(
                '%s charger offline at %s! This will show as non-responding in Homekit until the connection is restored.',
                chargerName,
                new Date(
                  charger.config_data.sync_timestamp * 1000
                ).toLocaleString()
              );
              break;
            case 0: //'Disconnected':
              this.log.warn(
                '%s disconnected at %s! This will show as non-responding in Homekit until the connection is restored.',
                chargerName,
                new Date(
                  charger.config_data.sync_timestamp * 1000
                ).toLocaleString()
